...
function gradeFor(shot: Shot, i: number): Grade {
  ...
  if (has("night", "midnight", "moon", "dark room", "starlit", "streetlight", "blackout"))
    return GRADES.night!;
  if (has("fire", "flame", "lantern", "ember", "candle", "torch", "dusk", "sunset", "furnace"))
    return GRADES.ember!;
  if (has("rain", "storm", "wet", "monsoon", "fog", "mist", "thunder")) return GRADES.storm!;
  if (has("angry", "fight", "blood", "scream", "fear", "threat", "battle", "knife", "corpse"))
    return GRADES.dread!;
  if (has("memory", "dream", "flashback", "vision", "hallucination")) return GRADES.memory!;
  if (has("indoor", "room", "kitchen", "lamp", "bulb", "hut", "shed")) return GRADES.interior!;
  if (has("cold", "rooftop", "hospital", "office", "school", "train", "morgue", "alley"))
    return GRADES.cold!;
